...
const AlertDialogPortal = ({ className, children, ...props }: AlertDialogPrimitive.AlertDialogPortalProps) => (
  <AlertDialogPrimitive.Portal className={cn(className)} {...props}>
    <div className="dark fixed inset-0 z-50 flex items-end justify-center sm:items-center">{children}</div>
...
>(({ className, children, ...props }, ref) => (
  <AlertDialogPrimitive.Overlay
    className={cn("bg-background/80 animate-in fade-in fixed inset-0 z-50 backdrop-blur-sm transition-opacity", className)}
    {...props}
    ref={ref}
  />
));
...
>(({ className, ...props }, ref) => (
  <AlertDialogPrimitive.Cancel
    ref={ref}
    className={cn(buttonVariants({ variant: "outline" }), "mt-2 text-white sm:mt-0", className)}
    {...props}
  />
));
